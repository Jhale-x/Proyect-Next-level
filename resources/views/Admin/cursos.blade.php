...
@section('content')
    ...
    <style>
        ...
        .course-bar {
            width: 8px;
            background: linear-gradient(180deg, #0a2d44 0%, #dc3545 100%);
            flex-shrink: 0;
        }
    ...
    <div class="container-fluid py-4">
        ...
        <div id="vista-listado">
            ...
            <div class="row g-3">
                @php
                    $cursos = [
                        ['Aritmética', 'Prof. Roberto Jara'],
                        ['Álgebra', 'Prof. Roberto Jara'],
                        ['Geometría', 'Prof. Carlos Villena'],
                        ['Trigonometría', 'Prof. Carlos Villena'],
                        ['Razonamiento Matemático', 'Prof. Alejandro Mendoza'],
                        ['Física', 'Prof. Ricardo Tello'],
                        ['Química', 'Prof. Luis Gómez'],
                        ['Biología', 'Prof. Ana Martínez'],
                        ['Lenguaje', 'Prof. Sofía Castro'],
                        ['Literatura', 'Prof. Sofía Castro'],
                        ['Razonamiento Verbal', 'Prof. Javier Luna'],
                        ['Inglés', 'Miss Brenda Wright'],
                        ['Historia del Perú', 'Prof. Marco Polo'],
                        ['Historia Universal', 'Prof. Marco Polo'],
                        ['Geografía', 'Prof. Elena Ramos'],
                        ['Economía', 'Prof. Pedro Castillo'],
                        ['Psicología', 'Prof. Sara Méndez'],
                        ['Educación Física', 'Prof. Sandro Rossi'],
                    ];
                @endphp
                @foreach ($cursos as $curso)
                    <div class="col-md-4 col-lg-3">
                        <div class="course-card shadow-sm"
                            onclick="abrirCurso('{{ $curso[0] }}', '{{ $curso[1] }}')">
            ...
            @if ($esDocente)
                <div id="seccion-calificar-btn" class="text-center py-5">
                    <div class="calificar-container shadow-sm">
                        ...
                        <p class="mt-4 text-muted small"><i class="bi bi-info-circle me-1"></i> El sistema promedia
                            automáticamente de 0-20 a escala de letras.</p>
                    </div>
                </div>

                <div id="seccion-excel" class="d-none">
                    <div id="excel-actions" class="mb-3 d-flex justify-content-between">
                        ...
                        <form id="import-form" action="" method="POST" enctype="multipart/form-data"
                            ...
                            <button class="btn btn-primary btn-sm"><i class="bi bi-upload"></i> Subir Excel</button>
                        </form>
                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($alumnos as $alumno)
                                    <tr>
                                        ...
                                        @for ($i = 0; $i < 6; $i++)
                                            <td><input type="number" class="input-nota" value="0" min="0"
                                                    max="20" oninput="calcularLetra(this)"></td>
                                        @endfor
                    ...
                    <div class="mt-4 d-flex justify-content-end gap-2 pb-5">
                        <button class="btn btn-outline-secondary px-4 fw-bold" onclick="cerrarRegistroExcel()">Cerrar
                            Registro</button>
                    ...
                    <div class="col-lg-11">
                        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
                            ...
                            <div class="card-body p-0">
                                <div class="row g-0">
                                    <div
                                        class="col-md-4 bg-light d-flex flex-column align-items-center justify-content-center p-5 border-end text-center">
                                        ...
                                        <div class="px-4 py-1 bg-primary text-white rounded-pill fw-bold small">LOGRO
                                            ESPERADO</div>
                                    </div>
                                    <div class="col-md-8 p-4">
                                        <h6 class="fw-bold mb-4"><i
                                                class="bi bi-graph-up-arrow me-2 text-primary"></i>Detalle de Actividades
                                        </h6>
                                        <div class="row g-3">
                                            @php
                                                $acts = [
                                            ...
                                            @foreach ($acts as $a)
                                                <div class="col-md-6 col-xl-4">
                                                    <div
                                                        class="p-3 border rounded-3 bg-white shadow-xs d-flex align-items-center justify-content-between h-100">
                                                        <div class="d-flex align-items-center">
                                                            <div
                                                                class="icon-shape {{ $a[3] }} text-white rounded-3 p-2 me-3 small">
                                                                <i class="bi {{ $a[2] }}"></i></div>
                                                            <div>
                                                                <p class="small text-muted mb-0"
                                                                    style="font-size: 0.7rem;">{{ $a[0] }}</p>
                                        ...
                                        <div
                                            class="mt-4 p-3 bg-blue-light rounded-3 border border-primary border-opacity-25 small">
                                            <i class="bi bi-info-circle-fill me-2 text-primary"></i>Las notas reflejan el
                                            avance actual calificado por el docente.
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="modal-content rounded-4 border-0 shadow">
                ...
                <div class="modal-body p-4">
                    ...
                    <div id="form-alumno-container">
                        <form class="row">
                            <div class="col-md-3 mb-3"><label class="form-label">Nombre</label><input type="text"
                                    class="form-control"></div>
                            <div class="col-md-3 mb-3"><label class="form-label">Apellido</label><input type="text"
                                    class="form-control"></div>
                            <div class="col-md-3 mb-3"><label class="form-label fw-bold text-primary">DNI
                                    (Usuario)</label><input type="text" id="dni_al"
                                    class="form-control border-primary" maxlength="8"></div>
                            <div class="col-md-3 mb-3"><label class="form-label fw-bold text-primary">F. Nac
                                    (Clave)</label><input type="date" id="fecha_al"
                            ...
                            <div class="col-md-3 mb-3"><label class="form-label">Usuario Generado</label><input
                                    type="text" id="user_al" class="form-control bg-light" readonly></div>
                            <div class="col-md-3 mb-3"><label class="form-label">Clave Generada</label><input
                                    type="text" id="pass_al" class="form-control bg-light" readonly></div>

                            <div class="col-12 mt-3"><button type="submit" class="btn btn-primary px-5">Finalizar
                                    Registro Alumno</button></div>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function abrirCurso(titulo, profe) {
            ...
            document.getElementById('banner-dinamico').style.backgroundImage =
                `url('${banners[Math.floor(Math.random() * banners.length)]}')`;
            cerrarRegistroExcel();
        }
