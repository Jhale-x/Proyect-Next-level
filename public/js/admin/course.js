    ...
    fetch(`/admin/salones/${idSalon}/detalle${queryCurso}`)
        ...
        .then((data) => {
            ...
            actividadesActuales.forEach((act) => {
                ...
                const fechaStr = act.fecha_entrega ? ` Â· ${act.fecha_entrega}` : '';
            ...
            if (!data.alumnos || data.alumnos.length === 0) {
                tablaAlumnos.innerHTML = `
                    <tr>
                        <td colspan="10" class="text-center text-muted">No hay alumnos en este salÃ³n</td>
                    </tr>
                `;
                return;
            }
...
function actualizarEncabezadosActividades() {
    document.querySelectorAll('.btn-editar-fecha').forEach((button) => {
        ...
        if (small) {
            const fechaStr = actividad.fecha_entrega ? ` Â· ${actividad.fecha_entrega}` : '';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Abrir modal de editar fecha/hora al hacer click en botÃ³n de cabecera
    document.addEventListener('click', function (e) {
        ...
        if (!modalElement) {
            console.error('No se encontrÃ³ el modal modalEditarFecha.');
            return;
        }
        ...
        btnGuardarFecha.addEventListener('click', function () {
            ...
            fetch(`${baseUrl}/${id}/fecha`, {
                ...
                .then((resp) => {
                    ...
                    // Recargar el detalle del salÃ³n para mostrar los nuevos datos
    ...
    if (btnAgregarSeccion && contenedorSecciones) {
        btnAgregarSeccion.addEventListener('click', function () {
            ...
            div.classList.add('input-group', 'mb-2');
            div.innerHTML = `
                <input type="text" name="secciones[]" class="form-control" placeholder="Nombre de la secciÃ³n" required>
