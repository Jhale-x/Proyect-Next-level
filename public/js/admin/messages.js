const AdminMessages = (() => {
    ...
    function init() {
        console.log("Mensajes Admin iniciado");
    }
            ...
            .then(data => {
                ...
                box.innerHTML = data.map(s => `
                    <div class="inbox-item"
                        onclick="AdminMessages.abrirSalon(${s.id_curso_salon})">
        ...
        fetch(`/admin/messages/conversaciones/${idSalon}`)
            ...
            .then(data => {
                ...
                box.innerHTML = data.map(c => `
                    <div class="inbox-item"
                        onclick="AdminMessages.abrirChat(${c.id}, '${c.tipo}')">
    ...
    function cargarChat() {
        ...
        fetch(`/admin/messages/chat/${salonActual}?usuario=${usuarioActual}&tipo=${tipoActual}`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(m => {
                    ...
                    const esAdmin = m.emisor?.tipo === "user";

                    return `
                        <div class="message-bubble ${esAdmin ? "outgoing" : "incoming"}">
                            <div class="bubble">
    ...
    function cargarUsuariosSalon(buscar = "") {
        ...
        fetch(`/admin/messages/usuarios/${salonActual}?${params}`)
            .then(r => r.json())
            .then(usuarios => {
                ...
                box.innerHTML = usuarios.map(u => `
                    <div class="user-item" 
                        onclick="AdminMessages.selectarUsuario(${u.id}, '${u.tipo}', '${u.nombre}')">
