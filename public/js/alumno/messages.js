const AlumnoMessages = (() => {
    ...
    function init() {
        console.log("Mensajes Alumno iniciado");
    }
    ...
    function cargarSalones(idCurso) {
        ...
        fetch(`/alumno/messages/curso/${idCurso}/salones`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(s => `
                    <div class="inbox-item"
                        onclick="AlumnoMessages.abrirSalon(${s.id_curso_salon})">
    ...
    function abrirSalon(idSalon) {
        ...
        fetch(`/alumno/messages/conversaciones/${idSalon}`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(c => `
                    <div class="inbox-item"
                        onclick="AlumnoMessages.abrirChat(${c.id}, '${c.tipo}')">
    ...
    function cargarChat() {
        ...
        fetch(`/alumno/messages/chat/${salonActual}?usuario=${usuarioActual}&tipo=${tipoActual}`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(m => {
                    ...
                    const esAlumno = m.emisor?.tipo === "user";

                    return `
                        <div class="message-bubble ${esAlumno ? "outgoing" : "incoming"}">
                            <div class="bubble">
    ...
    function sendMessage() {
        ...
        fetch("/alumno/messages/responder", {
            method: "POST",
            headers: {
    ...
    function cargarUsuariosSalon(buscar = "") {
        ...
        fetch(`/alumno/messages/usuarios/${salonActual}?${params}`)
            .then(r => r.json())
            .then(usuarios => {
                ...
                box.innerHTML = usuarios.map(u => `
                    <div class="user-item" 
                        onclick="AlumnoMessages.selectarUsuario(${u.id}, '${u.tipo}', '${u.nombre}')">
                        ...
                        <div class="user-info">
                            ...
                            <div class="user-role">${u.tipo === 'alumno' ? 'Alumno' : 'Alumno'}</div>
                        </div>
                    </div>
                `).join("");
