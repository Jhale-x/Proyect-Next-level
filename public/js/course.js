...
function abrirDetalle(idSalon) {
    ...
    // update excel forms if present
    const exportForm = document.getElementById('export-form');
    const importForm = document.getElementById('import-form');
    if (exportForm) {
        // export route requires both salón and curso IDs
        if (cursoActual) {
            exportForm.action = `/admin/salones/${idSalon}/export/${cursoActual}`;
        } else {
            exportForm.action = '';
        }
    }
    ...
    fetch(`/admin/salones/${idSalon}/detalle`)
        ...
        .then(data => {
            ...
            data.actividades.forEach(act => {
                head += `<th class="text-center">${act.actividad.actividad}</th>`;
            });
