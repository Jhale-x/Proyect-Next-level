const AuxiliarMessages = (() => {
    ...
    function init() {
        console.log("Mensajes Auxiliar iniciado");
    }
    ...
    function cargarSalones(idCurso) {
        ...
        fetch(`/auxiliar/messages/curso/${idCurso}/salones`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(s => `
                    <div class="inbox-item"
                        onclick="AuxiliarMessages.abrirSalon(${s.id_curso_salon})">
    ...
    function abrirSalon(idSalon) {
        ...
        fetch(`/auxiliar/messages/conversaciones/${idSalon}`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(c => `
                    <div class="inbox-item"
                        onclick="AuxiliarMessages.abrirChat(${c.id}, '${c.tipo}')">
    ...
    function cargarChat() {
        ...
        fetch(`/auxiliar/messages/chat/${salonActual}?usuario=${usuarioActual}&tipo=${tipoActual}`)
            .then(r => r.json())
            .then(data => {
                ...
                box.innerHTML = data.map(m => {
                    ...
                    const esAuxiliar = m.emisor?.tipo === "user";

                    return `
                        <div class="message-bubble ${esAuxiliar ? "outgoing" : "incoming"}">
                            <div class="bubble">
    ...
    function sendMessage() {
        ...
        fetch("/auxiliar/messages/responder", {
            method: "POST",
            headers: {
    ...
    function cargarUsuariosSalon(buscar = "") {
        ...
        fetch(`/auxiliar/messages/usuarios/${salonActual}?${params}`)
            .then(r => r.json())
            .then(usuarios => {
                ...
                box.innerHTML = usuarios.map(u => `
                    <div class="user-item" 
                        onclick="AuxiliarMessages.selectarUsuario(${u.id}, '${u.tipo}', '${u.nombre}')">
                        ...
                        <div class="user-info">
                            ...
                            <div class="user-role">${u.tipo === 'alumno' ? 'Alumno' : 'Auxiliar'}</div>
                        </div>
                    </div>
                `).join("");
